...
export class ContentDetector {
    ...
    xmlParser = new XmlContentParser();
    textParser = new TextContentParser();
    binaryParser = new BinaryContentParser();
    /**
    ...
    parseContent(content, options = {}) {
        const { forceType, strictTypeChecking = false, contentTypeHeader, encoding = 'utf8' } = options;
        try {
     ...
     */
    getQueryCapabilities(contentInfo) {
        return {
     ...
     */
    validateContentType(content, expectedType) {
        try {
            const detection = this.detect(content);
            const valid = detection.detectedType === expectedType;
            const errors = [];
            if (!valid) {
                errors.push(`Expected ${expectedType} but detected ${detection.detectedType} (confidence: ${detection.confidence})`);
            }
            return {
                valid,
     ...
     */
    getInitialGuess(contentTypeHeader, filename) {
     ...
     */
    combineDetectionResults(initial, analysis, strictChecking) {
     ...
     */
    createForcedContentInfo(forcedType) {
        const typeMap = {
            json: {
                contentType: 'application/json',
