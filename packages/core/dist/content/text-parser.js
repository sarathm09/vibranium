...
export class TextContentParser {
    ...
    parse(content, options = {}) {
        const { encoding = 'utf8', trimWhitespace = false, splitLines = false, removeEmptyLines = false } = options;
        try {
    ...
    search(text, pattern, options = {}) {
        const { caseSensitive = true, wholeWord = false, includeContext = false, contextLines = 1 } = options;
        try {
            ...
            lines.forEach((line, lineIndex) => {
                ...
                while ((match = searchRegex.exec(line)) !== null) {
                    ...
                    if (includeContext) {
                        result.context = this.extractContext(lines, lineIndex, contextLines);
                    }
                    results.push(result);
     ...
     */
    getLines(text, startLine, endLine) {
     ...
     */
    validateEncoding(buffer, expectedEncoding = 'utf8') {
        try {
     ...
     */
    extractContext(lines, targetLine, contextLines) {
