...
export class XmlContentParser {
     ...
     */
    query(document, xpathExpression, contextNode) {
        try {
     ...
     */
    queryValue(document, xpathExpression, defaultValue) {
        try {
     ...
     */
    pathExists(document, xpathExpression) {
        try {
            const results = this.query(document, xpathExpression);
     ...
     */
    getAllPaths(document, options = {}) {
        const { includeAttributes = false, maxDepth = 20 } = options;
        const paths = [];
        const traverse = (node, currentPath, depth) => {
            if (depth > maxDepth) {
                return;
            }
