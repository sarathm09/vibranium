...
export class ValidationEngine {
     ...
     */
    async validate(expectations, context) {
        ...
        for (const [operator, expected] of Object.entries(expectations)) {
            try {
                const result = await this.validateOperator(operator, expected, context);
                results.push(result);
            }
            catch (error) {
                results.push({
     ...
     */
    async validateOperator(operator, expected, context) {
        const { data } = context;
        switch (operator) {
            case 'status':
     ...
     */
    validateJsonPath(expected, data) {
        try {
            const results = [];
            let allPassed = true;
    ...
    deepEqual(expected, actual) {
        ...
        if (typeof expected === 'object') {
            ...
            return expectedKeys.every(key => actualKeys.includes(key) && this.deepEqual(expected[key], actual[key]));
        }
        return false;
    }
}
