...
export class BinaryContentParser {
  ...
  parse(content: Buffer, options: BinaryParseOptions = {}): BinaryParseResult {
    ...
    try {
      ...
      const result: BinaryParseResult = {
   ...
   */
  isTextContent(buffer: Buffer, sampleSize: number = 1024): boolean {
  ...
  extractAs(
    buffer: Buffer,
    format: 'base64' | 'hex' | 'utf8' | 'ascii' | 'binary'
  ): string {
    switch (format) {
      case 'base64':
   ...
   */
  private analyzeContent(buffer: Buffer): BinaryParseResult['metadata'] {
    ...
    return {
      ...
      isArchive: (
        fileType?.mimeType.includes('zip') ||
        fileType?.mimeType.includes('gzip') ||
        fileType?.mimeType.includes('7z') ||
        fileType?.mimeType.includes('bzip')
      ) || false,
