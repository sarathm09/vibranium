...
import { StepExecutor, PluginExecutor } from './step-executor';
...
export class ScenarioOrchestrator {
  ...
  async executeScenario(
    scenario: Scenario,
    initialVariables: Partial<ResolverContext> = {},
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < executionPlan.batches.length; batchIndex++) {
        ...
        const batchResults = await this.executeBatch(
          batch,
          context,
          Math.min(maxConcurrency, batch.length),
          { timeout, retries }
        );
   ...
   */
  validateScenario(scenario: Scenario): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
