...
export class JsonParser {
   ...
   */
  async parse(content: string, options: JsonParseOptions = {}): Promise<any> {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        const enhancedError = new Error(
          `JSON syntax error: ${error.message}${file ? ` in ${file}` : ''}`
        );
        
        // Add position information
        if (position) {
          (enhancedError as any).mark = position;
        }
   ...
   */
  stringify(data: any, options: { indent?: number; sorted?: boolean } = {}): string {
   ...
   */
  validateSyntax(content: string): { valid: boolean; error?: string; line?: number; column?: number } {
    try {
      JSON.parse(content);
      return { valid: true };
    } catch (error) {
   ...
   */
  private extractErrorPosition(message: string, content: string): { line: number; column: number } | null {
   ...
   */
  private validateStrictJson(content: string): void {
