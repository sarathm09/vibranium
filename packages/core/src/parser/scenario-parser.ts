...
export class ScenarioParser {
  ...
  async parse(
    ...
    try {
      ...
      const parseError: ScenarioParseError = {
   ...
   */
  async validate(scenario: Scenario): Promise<{ valid: boolean; errors: string[]; warnings: string[] }> {
   ...
   */
  private detectFormat(content: string, file?: string): 'yaml' | 'json' {
   ...
   */
  private normalizeScenario(data: any): Scenario {
