...
import type { Scenario, Step, Environment } from '../types';
...
export class SchemaValidator {
  private ajv: Ajv;
  private scenarioValidator?: ValidateFunction<Scenario>;
  private stepValidator?: ValidateFunction<Step>;
  private environmentValidator?: ValidateFunction<Environment>;
   ...
   */
  async validate(scenario: any): Promise<ValidationResult> {
    try {
   ...
   */
  async validateStep(step: any): Promise<ValidationResult> {
   ...
   */
  async validateEnvironment(environment: any): Promise<ValidationResult> {
  ...
  private addCustomKeywords(): void {
    ...
    this.ajv.addKeyword({
      ...
      type: 'string',
      schemaType: 'object',
      compile: (schema: { availableSteps?: string[] }) => {
        return (data: string) => {
  ...
  private compileValidators(): void {
    try {
      // Scenario schema
      const scenarioSchema: JSONSchemaType<Scenario> = {
        type: 'object',
        properties: {
   ...
   */
  private validateSemantics(scenario: Scenario): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
