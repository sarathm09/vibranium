...
export class YamlParser {
   ...
   */
  async parse(content: string, options: YamlParseOptions = {}): Promise<any> {
    ...
    try {
      ...
      if (document.errors.length > 0) {
        ...
        const yamlError = new Error(
          `YAML parsing error: ${error.message}${file ? ` in ${file}` : ''}`
        );
        
        // Add position information for better error reporting
        if (error.pos) {
          (yamlError as any).mark = {
            line: error.pos[0],
            column: error.pos[1]
          };
        }
      ...
      if (error instanceof yaml.YAMLParseError) {
        const enhancedError = new Error(
          `YAML syntax error: ${error.message}${file ? ` in ${file}` : ''}`
        );
        
        // Preserve original error details
        (enhancedError as any).mark = {
   ...
   */
  stringify(data: any, options: { indent?: number; flow?: boolean } = {}): string {
