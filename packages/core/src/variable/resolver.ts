...
import type { VariableContext, VariableMap } from '@vibraniumjs/types';
...
export class VariableResolver {
   ...
   */
  private parseReference(reference: string): { namespace: string; path: string[] } {
  ...
  private resolveValue(
    ...
    for (const segment of path) {
      ...
      if (current === null || current === undefined) {
        throw new Error(
          `Cannot access property '${segment}' of ${current} at path: $.${namespace}.${traversedPath.join('.')}`
        );
      }
      
      if (typeof current !== 'object') {
        throw new Error(
          `Cannot access property '${segment}' of non-object value at path: $.${namespace}.${traversedPath.slice(0, -1).join('.')}`
        );
      }
      ...
      if (Array.isArray(current)) {
        ...
        if (isNaN(index)) {
          throw new Error(
            `Invalid array index '${segment}' at path: $.${namespace}.${traversedPath.join('.')}`
          );
        }
