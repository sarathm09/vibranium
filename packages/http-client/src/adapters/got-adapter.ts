...
// Note: Got import disabled due to TypeScript module resolution issues
// The HTTP client factory will fall back to axios adapter automatically

// Temporary mock for Got types until module resolution is fixed
type GotOptions = {
  method?: string;
...
// Mock got instance that will cause factory to use axios instead
const got: Got = (() => {
  throw new Error('Got adapter requires "got" package to be installed. Please use axios or fetch adapter instead.');
}) as any;
got.extend = () => got;
