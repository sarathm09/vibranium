...
export class DataTypeGenerators {
    ...
    generateProduct() {
        ...
        return {
            ...
            tags: this.randomGen.fromArrayMultiple(['popular', 'new', 'sale', 'featured', 'bestseller', 'limited'], this.randomGen.number(1, 3))
        };
    }
    /**
     * Generate API response data
     */
    generateApiResponse(status = 'success') {
        if (status === 'error') {
            return {
                success: false,
                error: {
     ...
     */
    generateDatabaseRecord(tableName) {
        const baseRecord = {
            id: this.randomGen.uuid(),
