...
export class FileDiscovery {
    static DEFAULT_SCENARIO_EXTENSIONS = ['yaml', 'yml', 'json'];
    static DEFAULT_IGNORE_PATTERNS = ['node_modules/**', '.git/**', 'dist/**', 'build/**'];
    /**
     * Discover scenario files in a directory
     */
    static async discoverScenarios(directory, options = {}) {
        const { extensions = this.DEFAULT_SCENARIO_EXTENSIONS, recursive = true, maxDepth = 10, ignorePatterns = this.DEFAULT_IGNORE_PATTERNS } = options;
        const patterns = extensions.map(ext => recursive ? `**/*.${ext}` : `*.${ext}`);
        try {
     ...
     */
    static async findConfigFiles(startDirectory, configNames = ['scenariorest.config.json', 'vibranium.config.json', '.vibraniumrc']) {
        const found = [];
     ...
     */
    static async findEnvironmentFiles(directory) {
     ...
     */
    static async filterScenarioFiles(files) {
        const scenarioFiles = [];
     ...
     */
    static async isValidScenarioContent(content, filePath) {
        try {
     ...
     */
    static looksLikeScenario(content) {
     ...
     */
    static async watchDirectory(directory, callback, options = {}) {
