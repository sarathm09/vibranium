...
export class PatternGenerators {
  ...
  private mappings: Map<string, string | string[] | (() => string)> = new Map();
   ...
   */
  private resolvePatternChar(char: string): string {
    switch (char) {
      case '#':
   ...
   */
  private resolvePlaceholder(name: string): string {
   ...
   */
  private randomFromString(str: string): string {
   ...
   */
  private findMatchingParen(str: string, startIndex: number): number {
    let count = 1;
    let i = startIndex + 1;
   ...
   */
  private expandQuantifiers(pattern: string): string {
  ...
  private setupDefaultMappings(): void {
    ...
    this.mappings.set('loremSentence', () => {
      const words = Array.from({ length: this.randomNumber(5, 12) }, () => 
        this.resolvePlaceholder('loremWord')
      );
      return words.join(' ') + '.';
    });
  }
   ...
   */
  private setupCustomMappings(): void {
