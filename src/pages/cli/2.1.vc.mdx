...
* **run** or **r** : To run the Vibranium test cases available inside the workspace. Refer to [Running Tests](cli/2.4.vc_r.md) for more details. Details are also available by running the command `vc r --help`
* **list** or **l** : To list all the collections, scenarios and tests available. Refer to [Listing Tests](cli/2.2.vc_l.md) for more details. Details are also available by running the command `vc l --help`
* **setup** or **s** : To setup Vibranium. `vc s --help`. Refer to [Configuration](setup/1.4.configuration.md) for more details.
* **create** or **c** : To create new scenarios.`vc c --help`. Refer to [Creating Tests](cli/2.3.vc_c.md) for more details.
* **debug** or **d** : To debug the Vibranium installation and other issues. You would not need this command in most cases. [More details](/cli/2.5.vc_others.md)