---
title: Vibranium Config file
slug: /vibranium/setup/1.5.config_json
author: sarathm09
---
...
- **tests_directory**: The name of the directory inside the workspace, where all tests are stored, This is the directory in the workspace directory inside which you'll find the scenarios, payloads, schemas etc. Usually, it is named `tests` or `Vibranium-Tests`
  
- **accounts**: The different system/account/app details. You can define as many systems as needed as key-object pairs. The key specifies the system name and the value object specifies the system details. Supported authentication methods are `basic auth`, `client-credentials` and `no-auth`.
...
- **env_vars**: The global variables that can be used throughout all executions. Refer to [Variables](docs/3.3.variables.md) for more details
